...
| **接收侧去重 / exactly-once** | 去重是**外部供应商**的职责。分布式 exactly-once 需 2PC/对端配合，成本远超收益。 |
...
npm workspaces 单仓多包：`common/`（前后端共享类型，契约真相源）、`backend/`（Fastify + worker + SQLite）、`frontend/`（Vite+React 可观测面板），并保留 `configs/ docs/ scripts/ test/`。详见 `readme.md`。
